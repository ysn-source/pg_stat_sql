...
#define PGSSC_DUMP_FILE PGSTAT_STAT_PERMANENT_DIRECTORY "/pg_stat_everystmts.stat" 
...
#define PGSS_BSTAT_STATEMENTS false
#define PGSS_BSTAT_SQL true
...
static char *kindChars[] = {"I","P","E"};

#define KINDCHAR(x) \
	((char*)kindChars[Max(x+1,2)])
...
/*
 * The enh stats counters kept within pgssEntry.
 */
typedef struct EveryCounters
{
	char		*kind;	/*  kind of query */
	double		total_time;	/* total planning/execution time */
	TimestampTz	start;											 
	int64		rows;			/* total # of retrieved or affected rows */
	int64		shared_blks_hit;	/* # of shared buffer hits */
	int64		shared_blks_read;	/* # of shared disk blocks read */
	...
	int64		temp_blks_written;	/* # of temp blocks written */
	double		blk_read_time;	/* time spent reading, in msec */
	double		blk_write_time; /* time spent writing, in msec */ //	double		usage;			/* usage factor */
	int64		wal_records;	/* # of WAL records generated */
	int64		wal_fpi;		/* # of WAL full page images generated */
	uint64		wal_bytes;		/* total amount of WAL bytes generated */
} EveryCounters;
typedef struct pgssEveryEntry
{
	pgsscHashKey key;			/* hash key of entry - MUST BE FIRST */
	EveryCounters	counters;		/* the statistics for this query */
	Size		query_offset;	/* query text offset in external file */
	int			query_len;		/* # of valid bytes in query string, or -1 */
	int			encoding;		/* query text encoding */
	slock_t		mutex;			/* protects the counters only */
...
/* Links to shared memory state */
static pgssSharedState *pgssc = NULL;
static HTAB *pgssc_hash = NULL;
...
static int	pgss_every_max;		/* 5 time max statements to track */
static int	pgss_track_every;	/* every tracking level */
...
PG_FUNCTION_INFO_V1(pg_stat_sql);
PG_FUNCTION_INFO_V1(pg_stat_sql_reset);
...
static void pgss_store(const char *query, uint64 queryId,
					   ...
					   pgssStoreKind kind,
					   double total_time, Timestamp start, uint64 rows,
...
static void pg_stat_statements_internal(FunctionCallInfo fcinfo,
										...
										bool showtext);
static void
pg_stat_sql_internal(FunctionCallInfo fcinfo,
					pgssVersion api_version,
					bool showtext);																				
static Size pgss_memsize(void);
static Size pgssc_memsize(void);
static pgssEntry *entry_alloc(pgssHashKey *key, 
							  Size query_offset, int query_len,
							  int encoding, bool sticky);
static pgssEveryEntry *every_entry_alloc(pgsscHashKey *key, 
										Size query_offset, int query_len, 
										int encoding, bool sticky);
static void entry_dealloc(bool bEvery);
static int qtext_store(char *query, uint32 query_len,
						Size *query_offset, int *gc_count);
static char *qtext_load_file(Size *buffer_size);
static char *qtext_fetch(Size query_offset, uint32 query_len,
						 char *buffer, Size buffer_size);
static bool need_gc_qtexts(void);
static void gc_qtexts(bool bEvery);
static void entry_reset(Oid userid, Oid dbid, uint64 queryid);
static void every_entry_reset (Oid userid, Oid dbid, uint64 queryid);
...
static void dump_file_shutdown(bool bEvery);
static void dump_file_startup(bool bEvery);
...
_PG_init(void)
{
	...
	DefineCustomIntVariable("pg_stat_statements.track_every",
							 "Selects the maximum number of single statements statistics tracked by pg_stat_sql. (-1 to disable)",
							 NULL,
							 &pgss_track_every,
							 5000,
							 100,
	...
	pgss_every_max = pgss_track_every;

	EmitWarningsOnPlaceholders("pg_stat_statements");
	EmitWarningsOnPlaceholders("pg_stat_sql");
	 ...
	 */
	RequestAddinShmemSpace(pgss_memsize()+pgssc_memsize());
	RequestNamedLWLockTranche("pg_stat_statements",1);
	RequestNamedLWLockTranche("pg_stat_everystmts",1);
	...
	{
		dump_file_startup(PGSS_BSTAT_STATEMENTS);
		if (pgss_every_enabled())
			dump_file_startup(PGSS_BSTAT_SQL);
	}*/
}
...
pgss_shmem_startup(void)
{
	...
	if (pgss_every_enabled()) 
	{
		pgssc = ShmemInitStruct("pg_stat_everystmts", sizeof(pgssSharedState), &foundc);
		if (!foundc) 
		{
			pgssc->lock = &(GetNamedLWLockTranche("pg_stat_everystmts"))->lock;
			pgssc->cur_median_usage = ASSUMED_MEDIAN_INIT;
			pgssc->mean_query_len = ASSUMED_LENGTH_INIT;
			SpinLockInit(&pgssc->mutex);
			pgssc->extent = 0;
			pgssc->n_writers = 0;
			pgssc->gc_count = 0;
		}
		memset(&infoc, 0, sizeof(infoc));
		infoc.keysize = sizeof(pgsscHashKey);
		infoc.entrysize = sizeof(pgssEveryEntry);
		pgssc_hash = ShmemInitHash("pg_stat_sql hash",
								pgss_every_max, pgss_every_max,
								&infoc,
								HASH_ELEM | HASH_BLOBS);
	...
	/*
	 * If we're in the postmaster (or background process), set up a shmem
...
pgss_shmem_shutdown(int code, Datum arg)
{
	...
	dump_file_shutdown(PGSS_BSTAT_STATEMENTS);
	if (pgss_every_enabled())
		dump_file_shutdown(PGSS_BSTAT_SQL);
...
pgss_ExecutorEnd(QueryDesc *queryDesc)
{
		...
		pgss_enabled(exec_nested_level))
	{
		...
		at = GetCurrentTimestamp() - queryDesc->totaltime->total;
...
pgss_store(const char *query, uint64 queryId,
		   int query_location, int query_len,
		   pgssStoreKind kind,
		   double total_time, TimestampTz start,
		   uint64 rows,
		   const BufferUsage *bufusage,
		   const WalUsage *walusage,
		   pgssJumbleState *jstate)
{
	...
	if (!entry)
	{
		 ...
		 */
		if (stored<0 || pgss->gc_count != gc_count)
			stored = qtext_store(norm_query ? norm_query : query, query_len,
								 &query_offset, NULL);
		...
		if (do_gc)
		{
			gc_qtexts(PGSS_BSTAT_STATEMENTS);
			if (pgss_every_enabled())
				gc_qtexts(PGSS_BSTAT_SQL);
		}
	...
	if (pgss_every_enabled())
	{
		pgssEveryEntry *everyEntry;

		/* Prepare key for the every query hash */
		everyKey.dbid= key.dbid;
		everyKey.queryid= key.queryid;
		everyKey.userid = key.userid;
		if (entry->counters.calls[kind]==0)
			everyKey.count=USAGE_INIT;
		else if (kind!=PGSS_INVALID)
			everyKey.count=entry->counters.calls[kind];
		/* Update query_offset and query_len, if an entry already exists in pg_stat_statements */
		if (entry)
		{
			query_offset = entry->query_offset;
			query_len = entry->query_len;
		}
		LWLockAcquire(pgssc->lock, LW_SHARED);
		
		everyEntry = (pgssEveryEntry *) hash_search(pgssc_hash, &everyKey, HASH_FIND, NULL);
		
		if (!everyEntry) 
		{
			elog(DEBUG2,"storing query : %s",norm_query ? norm_query : query); 
			//elog(NOTICE, "create new entry n° %d(cnt:%d) at %d (len:%d) with %d",(int32) everyKey.queryid,everyKey.count, query_offset, query_len, encoding);
			LWLockRelease(pgssc->lock);
			LWLockAcquire(pgssc->lock, LW_EXCLUSIVE);
			everyEntry = every_entry_alloc(&everyKey, query_offset, query_len, encoding,false);
		}
		
		
		volatile pgssEveryEntry *e = (volatile pgssEveryEntry *) everyEntry;
		

		SpinLockAcquire(&e->mutex);
		
		if (start!=-1) 
			e->counters.start = start;
		e->counters.kind = KINDCHAR(kind);
...
pg_stat_statements(PG_FUNCTION_ARGS)
{
	...
	pg_stat_statements_internal(fcinfo, PGSS_V1_8, true);
							...
							bool showtext)
{
	...
	while ((entry = hash_seq_search(&hash_seq)) != NULL)
	{
		...
		{
			if (api_version >= PGSS_V1_2)
				values[i++] = Int64GetDatumFast((int32) queryid);
...
{
	bool		showtext = PG_GETARG_BOOL(0);
	pg_stat_sql_internal(fcinfo, PGSS_V1_8, true);
...
Datum
pg_stat_sql_reset(PG_FUNCTION_ARGS)
{
	Oid			userid;
	Oid			dbid;
	uint64		queryid;
	...
	every_entry_reset(userid, dbid, queryid);
					...
					bool showtext)
{
	...
	if (!pgss )
		ereport(ERROR,
				(errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
				 errmsg("pg_stat_statements must be loaded via shared_preload_libraries")));
	if (!pgss_every_enabled() || !pgssc || !pgssc_hash)
		ereport(ERROR,
				(errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
				 errmsg("pg_stat_statements.track_every not enabled or misconfigured")));
	/* check to see if caller supports us returning a tuplestore */
	if (rsinfo == NULL || !IsA(rsinfo, ReturnSetInfo))
		ereport(ERROR,
				(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
	...
	if (showtext)
	{
		 ...
		 * Here it is safe to examine extent and gc_count without taking the
		 * mutex.  Note that although other processes might change
		 * pgssc->extent just after we look at it, the strings they then write
		 * into the file cannot yet be referenced in the hashtable, so we
		 * don't care whether we see them or not.
		 *
		 * If qtext_load_file fails, we just press on; we'll return NULL for
			...
			pgss->gc_count != gc_count)
		{
			if (qbuffer)
				free(qbuffer);
			qbuffer = qtext_load_file(&qbuffer_size);
			elog(DEBUG2,"file content : %s",qbuffer);
		}
	}
	...
	LWLockAcquire(pgssc->lock, LW_SHARED);
	hash_seq_init(&hash_seq, pgssc_hash);
	while ((entry = hash_seq_search(&hash_seq)) != NULL)
	{
		Datum		values[PG_STAT_SQL_COLS];
		bool		nulls[PG_STAT_SQL_COLS];
		int			i = 0;
		EveryCounters	tmp;
		uint64		queryid = entry->key.queryid;
		...
		if (is_allowed_role || entry->key.userid == userid)
		{
			values[i++] = Int64GetDatumFast((uint32) queryid);
			...
			if (showtext)
			{
				char	   *qstr = qtext_fetch(entry->query_offset,
											   ...
											   qbuffer,
											   qbuffer_size);
				//elog(NOTICE,"fetch entry => %s(%d) at %d (len:%d) with %d",qstr,entry->key.count,entry->query_offset,entry->query_len,entry->encoding);
				if (qstr&&qstr!=NULL&&*qstr != 0)
				{
					char	   *enc;
		...
		/* copy counters to a local variable to keep locking time short */
		{
			volatile pgssEveryEntry *e = (volatile pgssEveryEntry *) entry;

			SpinLockAcquire(&e->mutex);
			tmp = e->counters;
			SpinLockRelease(&e->mutex);
		}
		values[i++] = CStringGetTextDatum(tmp.kind);
		if (tmp.start!=0)
			values[i++] = TimestampTzGetDatum(tmp.start);
		else
			nulls[i++] = true;
		...
		/* fail if you forget to update this assert */
		Assert(i == (api_version >= PGSS_V1_8) ? PG_STAT_SQL_COLS : -1  );
...
pgssc_memsize(void)
{
	...
	size = MAXALIGN(sizeof(pgssSharedState));
	size = add_size(size, hash_estimate_size(pgss_every_max, sizeof(pgssEveryEntry)));
			...
			bool sticky)
{
	...
	while (hash_get_num_entries(pgss_hash) >= pgss_max)
		entry_dealloc(PGSS_BSTAT_STATEMENTS);
...
static pgssEveryEntry *
every_entry_alloc(pgsscHashKey *key, Size query_offset, int query_len, int encoding,
			bool sticky)
{
	pgssEveryEntry  *entry;
	bool		found;
	...
	/* Make space if needed */
	while (hash_get_num_entries(pgssc_hash) >= pgss_every_max)
		entry_dealloc(PGSS_BSTAT_SQL);

	/* Find or create an entry with desired hash code */
	entry = (pgssEveryEntry *) hash_search(pgssc_hash, key, HASH_ENTER, &found);

	if (!found)
	{
		/* New entry, initialize it */
		//elog(NOTICE,"new entry alloc with key : %d and cnt : %d",key->queryid,key->count);
		/* reset the statistics */
		memset(&entry->counters, 0, sizeof(EveryCounters));
		/* set the appropriate initial usage count */
		//entry->counters.usage = sticky ? pgssc->cur_median_usage : USAGE_INIT;
		/* re-initialize the mutex each time ... we assume no one using it */
		SpinLockInit(&entry->mutex);
		/* ... and don't forget the query text metadata */
		Assert(query_len >= 0);
...
/*
 * qsort comparator for sorting into increasing start order
 */
static int
every_entry_cmp(const void *lhs, const void *rhs)
{
	double		l_usage = (*(pgssEveryEntry *const *) lhs)->counters.start;
	double		r_usage = (*(pgssEveryEntry *const *) rhs)->counters.start;
...
entry_dealloc(bool bEvery)
{
	...
	{
		entries = palloc(hash_get_num_entries(pgssc_hash) * sizeof(pgssEveryEntry *));
		hash_seq_init(&hash_seq, pgssc_hash);
	}
	i = 0;
	tottextlen = 0;
	...
	if (!bEvery)
		...
		qsort(entries, i, sizeof(pgssEveryEntry *), every_entry_cmp);
	...
	for (i = 0; i < nvictims; i++)
	{
		if (!bEvery)
			...
			hash_search(pgssc_hash, &entries[i]->key, HASH_REMOVE, NULL);
	}
...
Size qtext_writeInit(pgssSharedState *pgshmem,int query_len,int *gc_count) 
{	
	Size off;
	volatile pgssSharedState *s = (volatile pgssSharedState *) pgshmem;
...
int qtext_writeEnd(pgssSharedState *pgshmem) 
{
	volatile pgssSharedState *s = (volatile pgssSharedState *) pgshmem;
...
static int
qtext_store(char *query, uint32 query_len,
			Size *query_offset, int *gc_count)
{
	Size		off;
	int			fd;
	 ...
	 */
	off = qtext_writeInit(pgss,query_len,gc_count);
	if (pgss_every_enabled())
		off = qtext_writeInit(pgssc,query_len,gc_count);
	*query_offset = off;
	...
	if (pg_pwrite(fd, query, query_len+1, off) != query_len+1)
		goto error;
	//if (pg_pwrite(fd,"\0" , 1, off + query_len) != 1)				goto error;
	...
	qtext_writeEnd(pgss);
	if (pgss_every_enabled())
		qtext_writeEnd(pgssc);
	return off+query_len+1;
 ...
 */
static char *
qtext_fetch(Size query_offset, uint32 query_len,
			char *buffer, Size buffer_size)
{
	/* File read failed? */
	if (buffer == NULL)
...
{
	Size		extent;
	int 		limitperentry  = Max(512 * pgss_max,
									 512 * pgss_every_max);
	int 		limit50percent = Max(2 * pgss->mean_query_len * pgss_max,
									 2 * pgss->mean_query_len * pgss_every_max);
...
static void gc_qtexts(bool bEvery)
{
	...
	HASH_SEQ_STATUS hash_seq;
	pgssEntry  *entry;
	pgssEveryEntry *eEntry;
	Size		extent;
	int			nentries;
	HTAB *pgss_current_hash;

	if (!bEvery) {
		pgss_current_hash = pgss_hash;
	}
	else 
	{
		pgss_current_hash = pgssc_hash;
	}
	/*
	...
	hash_seq_init(&hash_seq, pgss_current_hash);
	if (bEvery==PGSS_BSTAT_STATEMENTS)	
	{
		while ((entry = hash_seq_search(&hash_seq)) != NULL)
		{
	...
	{
		while ((eEntry = hash_seq_search(&hash_seq)) != NULL)
		{
			int			query_len = eEntry->query_len;
			char	   *qry = qtext_fetch(eEntry->query_offset,
										query_len,
										qbuffer,
										qbuffer_size);
...
entry_reset(Oid userid, Oid dbid, uint64 queryid)
{
	...
	/* check whether pg_stat_sql still needs the qtext */
	if (pgss_every_enabled())
	{
		num_every_entries = hash_get_num_entries(pgssc_hash);
	}

	/* All entries are removed? */
	if (num_entries != num_remove || num_every_entries != 0)
		goto release_lock;
...
static void every_entry_reset (Oid userid, Oid dbid, uint64 queryid)
{
	...
	num_entries = hash_get_num_entries(pgssc_hash);
	...
	if (userid != 0 && dbid != 0 && queryid != UINT64CONST(0))
	{
		...
		/* Remove the key if exists */
		entry = (pgssEveryEntry *) hash_search(pgssc_hash, &key, HASH_REMOVE, NULL);
		if (entry)				/* found */
			num_remove++;
	}
	else if (userid != 0 || dbid != 0 || queryid != UINT64CONST(0))
	{
		/* Remove all entries. */
		hash_seq_init(&hash_eseq, pgssc_hash);
		while ((entry = hash_seq_search(&hash_eseq)) != NULL)
		{
			if ((!userid || entry->key.userid == userid) &&
				(!dbid || entry->key.dbid == dbid) &&
				(!queryid || entry->key.queryid == queryid))
			{
				hash_search(pgssc_hash, &entry->key, HASH_REMOVE, NULL);
				num_remove++;
			}
		}
	}
...
static void dump_file_startup(bool bEvery) {
	...
	if (bEvery==PGSS_BSTAT_STATEMENTS && !pgss_hash)
		ereport(ERROR,
				(errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
				 errmsg("Error on initializing pg_stat_statements shared memory")));

	
	if (bEvery==PGSS_BSTAT_SQL && !pgssc_hash)
		ereport(ERROR,
				(errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
				 errmsg("Error on initializing pg_stat_sql shared memory")));
	
	
	elog(NOTICE,"calling dump_file_startup()");
	
	/* Unlink query text file possibly left over from crash */ //here, check whether erase needed for PG_STAT_SQL
	unlink(PGSS_TEXT_FILE);
	...
	if (bEvery==PGSS_BSTAT_STATEMENTS) 
		...
		file = AllocateFile(PGSSC_DUMP_FILE , PG_BINARY_R);
	if (!file)
	{	
		if (errno != ENOENT)
			goto open_error;
		/* No existing persisted stats file, so we're done *///FreeFile(qFile);
		return;
	}
	...
	for (i = 0; i < num; i++)
	{
		...
		else 
		{
			...
			if (fread(&temp, sizeof(pgssEveryEntry), 1, file) != 1)
				goto read_error;
			...
			if (fread(buffer, 1, temp.query_len + 1, file) != temp.query_len + 1)
				goto eof_error;
			elog(DEBUG2,"query startup : %s",buffer);
			/* Should have a trailing null, but let's make sure */ 
			buffer[temp.query_len] = 0;


			query_offset = pgssc->extent;
			if (fwrite(buffer, 1, temp.query_len + 1, qFile) != temp.query_len + 1)	
				goto write_error;
			pgssc->extent += temp.query_len + 1;

			/* make the hashtable entry (discards old entries if too many) */
			entry = every_entry_alloc(&temp.key, query_offset, temp.query_len,
								temp.encoding,
								false);
			/* copy in the actual stats */
			entry->counters = temp.counters;
	...
	ereport(LOG,
			(errcode_for_file_access(),
			 errmsg("could not open file \"%s\": %m",
					(!bEvery)?PGSS_DUMP_FILE :PGSSC_DUMP_FILE)));
	goto fail;
eof_error:
	ereport(LOG,
			(errcode_for_file_access(),
			 errmsg("could not read eof \"%s\": %m",
					(!bEvery)?PGSS_DUMP_FILE :PGSSC_DUMP_FILE)));
	goto fail;
header_error:
	ereport(LOG,
			(errcode_for_file_access(),
			 errmsg("bad header for file \"%s\": %m",
					(!bEvery)?PGSS_DUMP_FILE :PGSSC_DUMP_FILE)));
	goto fail;
read_error:
	ereport(LOG,
	...
	ereport(LOG,
			(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
			 errmsg("ignoring invalid data in file \"%s\"",
					(!bEvery)?PGSS_DUMP_FILE ".tmp":PGSSC_DUMP_FILE ".tmp")));
	goto fail;
write_error:
	ereport(LOG,
			(errcode_for_file_access(),
...
static void dump_file_shutdown(bool bEvery)
{
	...
	{
		file = AllocateFile(PGSSC_DUMP_FILE ".tmp", PG_BINARY_W);
		pgss_current_hash = pgssc_hash;
	}
	if (!file)
		goto file_error;
	...
	if (fwrite(&PGSS_PG_MAJOR_VERSION, sizeof(uint32), 1, file) != 1)
		goto data_error;
	num_entries = hash_get_num_entries(pgss_current_hash);
	...
	else
	{
		...
		while ((entry = hash_seq_search(&hash_seq)) != NULL)
		{
			...
			char	   *qstr = qtext_fetch(entry->query_offset, len,
										qbuffer, qbuffer_size);
			elog(DEBUG2,"check shutdown of qstr : %s",qstr);
			if (qstr == NULL)
				continue;			/* Ignore any entries with bogus texts */

			if (fwrite(entry, sizeof(pgssEveryEntry), 1, file) != 1 ||
				fwrite(qstr, 1, len + 1, file) != len + 1)
			{
				/* note: we assume hash_seq_term won't change errno */
				hash_seq_term(&hash_seq);
	...
	//if (FreeFile(file))	{		file = NULL;		goto file_error;	}
	...
	if (bEvery==PGSS_BSTAT_STATEMENTS) 
		...
		(void) durable_rename(PGSSC_DUMP_FILE ".tmp", PGSSC_DUMP_FILE, LOG);
...
qtext_error:
	ereport(LOG,
			(errcode_for_file_access(),
			 errmsg("could not load file \"%s\": %m",	PGSS_TEXT_FILE)));
done:
	if (qbuffer) 
	{
		free(qbuffer);
	...
	if (file)
		FreeFile(file);
	//if (bEvery==PGSS_BSTAT_STATEMENTS) 		unlink(PGSS_DUMP_FILE);	else		unlink(PGSSC_DUMP_FILE);
}